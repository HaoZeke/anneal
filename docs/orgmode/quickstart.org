...
GLE-Langevin (requires gradient — see :eindir:doc:`howto/gradients` for how to supply native gradients from Python/torch/JAX or analytic code; colored noise via eindir thermostat):
