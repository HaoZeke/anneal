...
=run_device= and =run_ensemble= are thin shims around the same Rust runner that =run= uses. The transition kernel they execute is the five-component Sampler object (Obj + Cool + Neigh + Move + Accept) that the presets and advanced drivers construct. The batched device path only adds a leading axis to state and to the objective/gradient evaluations; the acceptance logic, cooling schedule, and proposal generation use the same code paths. Consequently the noise-aware compensated Accept, the generalized =Langevin= equation (generalized =Langevin= equation (GLE)) Move, and the rank-1 independence Move are available on GPU and in large ensembles when the same slots are selected.
