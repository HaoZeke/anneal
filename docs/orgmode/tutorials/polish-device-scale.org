...
After any stochastic driver you run a deterministic "last mile". The same polish component also works as a pure multi-start method (quasi-Monte Carlo (quasi-Monte Carlo (quasi-Monte Carlo (QMC))) or shifted-quasi-Monte Carlo (quasi-Monte Carlo (QMC))). The device path (NumPy or CuPy) and the batched ensemble runner execute the identical transition kernel; only the array namespace and leading batch axis change.
...
=run_device= and =run_ensemble= are thin shims around the same Rust runner that =run= uses. The transition kernel they execute is the five-component Sampler object (Obj + Cool + Neigh + Move + Accept) that the presets and advanced drivers construct. The batched device path only adds a leading axis to state and to the objective/gradient evaluations; the acceptance logic, cooling schedule, and proposal generation are identical bit-for-bit. Consequently the noise-aware compensated Accept, the generalized =Langevin= equation (generalized =Langevin= equation (GLE)) Move, the rank-1 independence Move, and every future improvement to any slot are automatically present on GPU and in large ensembles.

quasi-Monte Carlo (quasi-Monte Carlo (QMC)) polish is likewise a pure filling of the same slots (a deterministic Cool that goes to zero, a Move that is projected gradient, an Accept that is ==Armijo==). One change to the ==Armijo== logic or the low-discrepancy generator instantly appears in every polish variant.
