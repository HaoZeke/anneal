...
impl Surface {
    fn value(&self, x: ArrayView1<f64>) -> f64 {
        match self.landscape {
            Landscape::Rastrigin => x.iter().map(|v| {
                v * v + 10.0 * (1.0 - (std::f64::consts::TAU * v).cos())
            }).sum(),
            Landscape::ConditionedQuadratic => x.iter().enumerate().map(|(j, v)| {
                0.5 * self.curvature(j) * v * v
            }).sum(),
        }
    }
...
impl Gradient<f64> for Surface {
    fn grad(&self, x: ArrayView1<f64>) -> Array1<f64> {
        ...
        match self.landscape {
            ...
            Landscape::ConditionedQuadratic => Array1::from_iter(
                x.iter().enumerate().map(|(j, v)| self.curvature(j) * v)
            ),
        }
    }
...
fn main() {
    ...
    let number = |index: usize, default: usize| {
        args.get(index).map(|s| s.parse::<usize>().expect("positive integer argument")).unwrap_or(default)
    };
    let dim = number(1, 32);
    ...
    assert!(std::env::var_os("HISTORY_NNG").is_none(), "this control measures in-process shared history");
    let settings = GleEscapeConfig { steps, ..GleEscapeConfig::default() };
    let modes = [
        ("gaussian", BoxEscape::Gaussian),
        ("white", BoxEscape::Langevin(GleEscapeConfig {
            noise: GleNoise::White { friction: 4.0 }, ..settings
        })),
    ...
    for landscape in [Landscape::Rastrigin, Landscape::ConditionedQuadratic] {
        for seed in 0..seeds as u64 {
            for (noise, escape) in modes {
                for (history_name, history) in [("private", HistoryMode::Private), ("shared", HistoryMode::Shared)] {
                    let surface = Surface {
                        landscape,
                    ...
                    let config = BoxEnsembleConfig { budget, history, escape, ..BoxEnsembleConfig::default() };
                    let began = Instant::now();
                    let result = box_ensemble_optimize(&surface, &surface, seed, Some(start.view()), &config);
