...
fn expected_sources(walkers: usize, reader: usize, neighbors: usize) -> Vec<usize> {
    let steps = if neighbors == 0 {
        walkers - 1
    } else {
        neighbors.min(walkers - 1)
    };
        ...
        for neighbors in [0, 1, 2, usize::MAX] {
            ...
            for reader in 0..walkers {
                ...
                let samples: Vec<_> = sources
                    .iter()
                    .map(|&source| (source, array![source as f64]))
                    .collect();
                let visits: Vec<_> = sources
                    .iter()
                    .map(|&source| (array![source as f64], source as u64 + 1))
                    .collect();
                assert_eq!(
                    exchange.drain_samples(reader),
                    samples,
                    "walkers={walkers}, reader={reader}, neighbors={neighbors}"
                );
            ...
            assert_eq!(
                exchange.sample_counts(),
                (walkers as u64, delivered_samples)
            );
            assert_eq!(
                exchange.counts(),
                (((walkers * (walkers + 1)) / 2) as u64, delivered_visits)
            );
...
fn explicit_zero_preserves_the_default_exchange() {
    ...
    for step in 0..16 {
        ...
        for reader in (0..5).rev() {
            assert_eq!(
                default.drain_samples(reader),
                explicit.drain_samples(reader)
            );
