...
impl Plateau {
    fn new(finite: bool) -> Self {
        Self {
            bounds: Bounds::new(array![-1000.0, 4.0, -0.002], array![3000.0, 4.0, 0.006], 0.0),
    ...
    fn run(&self, gradient: bool, config: &BoxEnsembleConfig, coverage: &BoxCoverageConfig) -> BoxEnsembleResult {
        ...
        assert_eq!(result.shared_deposits, result.coverage.applied_foreign_visits);
        result
    }
}
...
impl Objective<f64> for Plateau {
    fn eval(&self, x: ArrayView1<f64>) -> f64 {
        assert_eq!(x.len(), 3);
        for (x, (lo, hi)) in x.iter().zip(self.bounds.low.iter().zip(self.bounds.high.iter())) {
...
fn config() -> BoxEnsembleConfig {
    BoxEnsembleConfig { replicas: 4, budget: 512, history: HistoryMode::None, ..BoxEnsembleConfig::default() }
}
...
fn zero_coverage_height_preserves_the_unbiased_search_path() {
    for gradient in [false, true] {
        ...
        let coverage = BoxCoverageConfig { height: 0.0, ..BoxCoverageConfig::default() };
        let first = shared.run(gradient, &config(), &coverage);
        let second = private.run(gradient, &config(), &BoxCoverageConfig { shared: false, ..coverage });
        assert_eq!(*shared.trace.lock().unwrap(), *private.trace.lock().unwrap());
        assert_eq!((first.n_evals, first.n_grads, first.hops), (second.n_evals, second.n_grads, second.hops));
...
fn coverage_radius_is_normalized_and_peer_caps_apply_per_receiving_region() {
    for gradient in [false, true] {
        let objective = Plateau::new(true);
        let config = BoxEnsembleConfig { shared_deposits: 1, ..config() };
        let coverage = BoxCoverageConfig { radius: 2.0, ..BoxCoverageConfig::default() };
...
fn minimum_match_tolerance_does_not_set_coverage_reach() {
    for gradient in [false, true] {
        ...
        let first = tight.run(gradient, &BoxEnsembleConfig { identity_tol: 1e-12, ..config() }, &coverage);
        let second = loose.run(gradient, &BoxEnsembleConfig { identity_tol: 1.0, ..config() }, &coverage);
