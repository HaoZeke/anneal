...
use anneal_core::methods::portfolio::{
    PortfolioEnsembleConfig, portfolio_values_ensemble_optimize,
};
...
fn scalar_portfolio_delivers_only_funded_ring_neighbors() {
    for (replicas, neighbors, delivered) in [
        (2, 1, 2),
        (3, 1, 6),
        (5, 0, 20),
        (5, 1, 10),
...
fn scalar_hops_restrict_samples_and_deposits_with_no_minimum_history() {
    for (neighbors, delivered) in [(0, 30), (1, 15), (2, 30), (usize::MAX, 30)] {
        ...
        let result =
            box_values_ensemble_optimize_with_coverage(&loss, 17, None, &config, &coverage);
        assert_eq!(result.n_evals, loss.trace().len());
