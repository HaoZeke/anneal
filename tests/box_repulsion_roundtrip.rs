...
fn distance(a: ArrayView1<f64>, b: ArrayView1<f64>, bounds: &Bounds<f64>) -> f64 {
    ...
    let normalized = |point: ArrayView1<f64>, j: usize| {
        ((point[j] - bounds.low[j]) / widths[j]) * scale
    };
    widths.iter().enumerate().filter(|(_, w)| **w > 0.0)
        .map(|(j, _)| (normalized(a, j) - normalized(b, j)).powi(2))
        .sum::<f64>().sqrt()
}

fn clearance(point: ArrayView1<f64>, peers: &[Array1<f64>], bounds: &Bounds<f64>) -> f64 {
    peers.iter().map(|peer| distance(point, peer.view(), bounds))
...
fn rounded_case(fixed: bool) -> (Array1<f64>, Array1<f64>, Vec<Array1<f64>>, Bounds<f64>, f64) {
    ...
    let point = |a: f64, b: f64| {
        if fixed { array![low + a * spacing, 7.0, low + b * spacing] }
        else { array![low + a * spacing, low + b * spacing] }
    };
    let bounds = Bounds::new(point(0.0, 0.0), point(4.0, 6.0), 0.0);
    let peers = vec![point(1.0, 2.0), point(4.0, 1.0), point(1.0, 3.0), point(0.0, 1.0)];
    ...
    let mut coverage = coverage::Coverage::new(
        &bounds, 2, &coverage::BoxCoverageConfig {
            radius, ..coverage::BoxCoverageConfig::default()
        }, 8,
    );
    for peer in &peers {
    ...
    if fixed { assert_eq!(proposal[1], 7.0); }
...
fn representable_parameter_repulsion_increases_actual_peer_clearance() {
    for fixed in [false, true] {
        ...
        assert!(after > before, "physical clearance decreased: {before} -> {after}");
    }
}
...
fn representable_parameter_repulsion_keeps_the_geometric_step_cap() {
    for fixed in [false, true] {
        ...
        assert!(displacement <= allowance + 16.0 * f64::EPSILON,
