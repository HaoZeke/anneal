...
impl Flat {
    fn new(bounds: Bounds<f64>) -> Self {
        Self { bounds, points: Mutex::new(Vec::new()), gradients: AtomicUsize::new(0) }
    }
}
...
fn run(bounds: &Bounds<f64>, start: &Array1<f64>, shared: bool, values: bool) -> Run {
    ...
    let config = BoxEnsembleConfig {
        replicas: 4, budget: 64, history: HistoryMode::None,
        ..BoxEnsembleConfig::default()
    };
    let coverage = BoxCoverageConfig { shared, radius: 0.36, ..BoxCoverageConfig::default() };
    let result = if values {
        box_values_ensemble_optimize_with_coverage(
            &objective, 133, Some(start.view()), &config, &coverage,
        )
    } else {
        box_ensemble_optimize_with_coverage(
            &objective, &objective, 133, Some(start.view()), &config, &coverage,
        )
    };
    ...
    if values { assert_eq!(result.n_grads, 0); }
...
fn rounded_case(fixed: bool, values: bool) -> (Array1<f64>, Array1<f64>, Vec<Array1<f64>>, Bounds<f64>, f64) {
    let low = 1e10;
    ...
    assert_eq!(peers, vec![point(0.0, 2.0), point(4.0, 6.0), point(3.0, 4.0)]);
    let private = if values { run(&bounds, &start, false, true) } else { reference };
    ...
    assert_eq!(&private.points[..initialization.len()], initialization.as_slice());
    assert_eq!(&shared.points[..initialization.len()], initialization.as_slice());
