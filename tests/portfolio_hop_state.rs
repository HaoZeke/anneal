    ...
    fn grad(&self, x: ArrayView1<f64>) -> Array1<f64> {
        ...
        Array1::from_iter(x.iter().enumerate().map(|(j, &coordinate)| {
            if self.terraced {
                ...
                let shifted = coordinate - 0.271 - 0.01 * j as f64;
                2.0 * shifted
                    + 10.0 * std::f64::consts::TAU * (std::f64::consts::TAU * shifted).sin()
            }
        }))
    }
}
...
impl Gradient<f64> for NoGradient {
    fn dim(&self) -> usize {
        panic!("no gradient capability is supplied")
    }
    fn grad(&self, _: ArrayView1<f64>) -> Array1<f64> {
        panic!("no gradient callback is supplied")
    }
}
