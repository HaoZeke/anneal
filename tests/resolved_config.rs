...
fn resolved_configuration_is_stable_and_mode_sensitive() {
    ...
    let json_a = flexible.resolved_json().expect("serialize resolved configuration");
    let json_b = flexible.resolved_json().expect("repeat resolved configuration");
    ...
    assert_ne!(flexible.resolved_sha256().unwrap(), rigid.resolved_sha256().unwrap());

    let mut off = flexible;
    off.soap_mode = SoapProposalMode::Off;
    assert!(off.resolved_json().unwrap().contains(r#""soap_mode":"off""#));
    assert_ne!(rigid.resolved_sha256().unwrap(), off.resolved_sha256().unwrap());
}
