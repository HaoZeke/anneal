...
use anneal_core::{PortfolioEnsembleConfig, PortfolioEnsembleResult, portfolio_values_ensemble_optimize};
...
fn bounds() -> Bounds<f64> {
    Bounds::new(Array1::from_elem(DIM, -2.0), Array1::from_elem(DIM, 2.0), 0.0)
}
...
impl Objective<f64> for ScalarTrace {
    ...
    fn eval(&self, position: ArrayView1<f64>) -> f64 {
        ...
        self.traces.lock().unwrap().entry(std::thread::current().id())
            .or_default().push(position.to_owned());
...
fn run(shared: bool) -> (PortfolioEnsembleResult, Vec<Vec<Array1<f64>>>) {
    ...
    let traces: Vec<_> = objective.traces.into_inner().unwrap().into_values().collect();
    assert_eq!(traces.len(), config.replicas);
    assert!(traces.iter().all(|trace| trace.len() == config.budget / config.replicas));
...
fn strategy_start(traces: &[Vec<Array1<f64>>], replica: usize) -> (usize, usize, u64) {
    ...
    let starts = shifted_low_discrepancy_points(
        &bounds(), 1, anneal_core::qmc_skip_from_seed(gsa_seed), gsa_seed,
    );
    let found: Vec<_> = traces.iter().enumerate().flat_map(|(trace, positions)| {
        let start = starts.row(0);
        positions.iter().enumerate().filter_map(move |(index, position)| {
            (position.view() == start).then_some((trace, index))
        })
    }).collect();
    assert_eq!(found.len(), 1, "GSA initialization must be uniquely identified");
    ...
    (0..2 * DIM).map(|step| {
        ...
        let raw = if step < DIM {
            ...
            let visited = kernel.propose(
                ArrayView1::from(std::slice::from_ref(&anchor[axis])), 5230.0, &mut rng,
            );
...
fn private_scalar_gsa_replays_full_and_coordinate_strategy() {
    ...
    for replica in 0..2 {
        ...
        for (step, proposal) in expected.iter().enumerate() {
            assert_eq!(traces[which][start + step + 1], *proposal, "replica={replica}, step={step}");
        }
    }
}
...
fn shared_scalar_gsa_preserves_coordinate_support_with_effective_separation() {
    ...
    for replica in 0..2 {
        ...
        let matching: Vec<_> = shared.iter().filter(|trace| trace[0] == private[which][0]).collect();
        assert_eq!(matching.len(), 1, "replicas must retain their explicit starts");
        ...
        for axis in 0..DIM {
            ...
            for inactive in 0..DIM {
                if inactive != axis {
                    assert_eq!(proposal[inactive], anchor[inactive],
    ...
    assert!(coordinate_corrections > 0, "coordinate separation must not be disabled");
}
