...
fn face_separation(fixed_axis: bool) {
    ...
    let (separation, overlaps) = samples
        .separate(
            point.view(),
            point.view(),
            widths.view(),
...
#[test]
fn a_box_face_allows_sideways_separation() {
    face_separation(false);
}

#[test]
fn sideways_separation_preserves_fixed_coordinates() {
...
fn an_admissible_radial_correction_keeps_its_direction_and_random_state() {
    ...
    assert!(overlaps);
    let Separation::Moved(moved) = separation else {
        panic!("radial separation is feasible");
    };
