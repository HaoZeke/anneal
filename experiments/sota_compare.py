    ...
    try:
        while True:
            ...
            for _ in range(sa_block):
                ...
                fy = counter(y)
                if fy < vals[i] or rng.random() < np.exp(
                    -(fy - vals[i]) / max(temp, 1e-12)
                ):
...
def plain_sa(counter, low, high, dim, rng):
    ...
    return (
        hybrid_bmsa(counter, low, high, dim, rng, n_polish=0)
        if False
        else _plain(counter, low, high, dim, rng)
    )
...
def compare(dims=(10,), seeds=6, budget=6000):
    names = list(METHODS)
    print(
        f"best-at-budget ({budget} evals), median over {seeds} seeds; lower is better"
    )
