...
def verify(control, path, reference):
    ...
    for row in rows:
        ...
        assert d["drawn_disagreements"] <= min(
            d["drawn_comparisons"], d["probability_changes"]
        )
    ...
    if control == "overlap":
        assert (
            sum(row["coverage_decisions"]["probability_changes"] for _, row in pairs)
            > 0
        )
    if reference:
    ...
    for landscape in sorted({row["landscape"] for row in rows}):
        for noise in sorted({row["noise"] for row in rows}):
            selected = [
                (p, s)
                for p, s in pairs
                if s["landscape"] == landscape and s["noise"] == noise
            ]
            ...
            summary["groups"].append(
                {
                    ...
                    **{name: sum(d[name] for d in ds) for name in COUNTS},
                    "probability_change_sum": sum(
                        d["probability_change_sum"] for d in ds
                    ),
                    **{
                        name: max(d[name] for d in ds)
                        for name in MEASURES
                        if name != "probability_change_sum"
                    },
                    "shared_better": sum(
                        s["best_value"] < p["best_value"] for p, s in selected
                    ),
                    "equal": sum(
                        s["best_value"] == p["best_value"] for p, s in selected
                    ),
