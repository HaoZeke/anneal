...
def test_sa_run_is_deterministic():
    r1 = sa_run(objective="styb_tang_2d", variant="boltzmann",
                dtype="float64", seed=7, n_epochs=20, steps_per_epoch=50)
    r2 = sa_run(objective="styb_tang_2d", variant="boltzmann",
                dtype="float64", seed=7, n_epochs=20, steps_per_epoch=50)
...
def test_dtype_is_honoured(dtype):
    r = sa_run(objective="styb_tang_2d", variant="boltzmann",
               dtype=dtype, seed=1, n_epochs=10, steps_per_epoch=20)
...
def test_all_variants_run(variant):
    r = sa_run(objective="styb_tang_2d", variant=variant,
               dtype="float64", seed=3, n_epochs=20, steps_per_epoch=50)
    assert r.n_calls > 0
...
def test_compensated_delta_e_changes_trajectory():
    ...
    r_plain = sa_run(objective="styb_tang_2d", variant="boltzmann",
                     dtype="float64", seed=5, n_epochs=20, steps_per_epoch=50,
                     compensated_delta_e=False)
    r_comp = sa_run(objective="styb_tang_2d", variant="boltzmann",
                    dtype="float64", seed=5, n_epochs=20, steps_per_epoch=50,
...
def test_objective_registry_has_four_entries():
    assert set(OBJECTIVES.keys()) == {
        "styb_tang_2d", "rosenbrock_2d", "rastrigin_2d", "ackley_2d"
    }
