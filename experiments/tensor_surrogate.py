...
class AdditiveSurrogate:
    ...
    def sample(self, n: int, T: float, rng, grid_m: int = 65) -> np.ndarray:
        ...
        for j in range(dim):
            ...
            w = xs[1] - xs[0]
            out[:, j] = np.clip(
                xs[idx] + (rng.random(n) - 0.5) * w, self.low[j], self.high[j]
            )
        return out
...
class TensorTrainSurrogate:
    ...
    def build(
        ...
        *,
        ...
        rng = np.random.default_rng(rng)
        k = min(k, dim)
        rf = fit_reduced_surrogate(
            fn, grad, low, high, dim, k=k, degree=degree, n_pilot=n_pilot, rng=rng
        )
        grids = [
            np.linspace(rf.reduced_low[j], rf.reduced_high[j], grid_m) for j in range(k)
        ]
        mesh = np.meshgrid(*grids, indexing="ij")
        pts = np.stack([m.reshape(-1) for m in mesh], axis=1)
        f_grid = np.array([rf.surrogate.eval(r) for r in pts]).reshape([grid_m] * k)
...
# ---------------------------------------------------------------------------
def tt_independence_sa(
    prob, seed, max_fevals, *, k=3, degree=6, grid_m=17, local_frac=0.25, n_epochs=40
):
    ...
    surr = TensorTrainSurrogate.build(
        prob.fn, grad, low, high, dim, k=k, degree=degree, grid_m=grid_m, rng=rng
    )
...
def additive_independence_sa(
    prob, seed, max_fevals, *, degree=8, grid_m=65, local_frac=0.2, n_epochs=40
):
...
def _self_test() -> int:
    ...
    cube = np.einsum("ij,k->ijk", a, rng.standard_normal(m)) + np.einsum(
        "i,jk->ijk", rng.standard_normal(m), b
    )
    ...
    print(
        f"[1] TT roundtrip rel-err {rel:.2e}; ranks {ranks}; "
    ...
    print(
        f"    {'problem':14s} {'budget':>6s} {'rank1':>6s} {'TT-sub':>7s} "
        f"{'RWM':>5s}   medians (rank1 / sub / RWM / f*)"
    )
    ok3 = True
    for name, (toy, fstar) in [
        ("stybtang-d20", _styblinski_tang(20)),
        ...
        print(
            f"    {name:14s} {budget:6d} {mf_hits:4d}/20 {tt_hits:4d}/20 "
            f"{rwm_hits:3d}/20   "
        ...
        ok3 = ok3 and (
            mf_hits >= max(rwm_hits, 1) and np.median(mf_best) <= np.median(rwm_best)
        )

    print("\n--- ledger ---")
    for tag, ok in [
        ("1 TT roundtrip + compression", ok1),
