...
def worker(path, seed, budget, n, mode):
    ...
    require(
        all(int(value) == 0 for value in policies[0]),
        f"{path}: geometry policy executed",
    )
    ...
    require(
        math.isfinite(best) and best >= -n * (n - 1) / 2 - 1e-5,
        f"{path}: invalid Lennard-Jones objective value",
    )
    require(
        f"LJ{n}, budget {budget} charged evaluations, 1 seeds," in text,
    ...
    require(
        all(type(value) is int and value >= 0 for value in draws),
        f"{path}: invalid draws",
    )
    require(sum(draws) == int(blocks), f"{path}: local block count mismatch")
    require(
        all(type(value) in (int, float) and math.isfinite(value) for value in means),
        f"{path}: nonfinite surface rewards",
    )
    peers = int(peers)
    require(
        mode != "private" or peers == 0, f"{path}: private arm imported peer rewards"
    )
    charges = []
...
def summarize(root):
    ...
    for line in (root / "manifest.txt").read_text().splitlines():
        ...
        if separator:
            manifest[key] = value
    require(
        manifest.get("channels") == "surface-evidence-only",
        "wrong communication channels",
    )
    parameters = {
        key: int(manifest[key])
        for key in ("n", "budget", "replicas", "ensembles", "block")
    }
    require(
        all(value > 0 for value in parameters.values()), "positive parameters required"
    )
    budget, replicas, ensembles = (
        parameters[key] for key in ("budget", "replicas", "ensembles")
    )
    best, hits, workers = {}, {}, {}
    for mode in ("private", "shared"):
        ...
        for ensemble in range(ensembles):
            ...
            if mode == "shared":
                require(
                    sum(row["peer_blocks"] for row in records) > 0,
