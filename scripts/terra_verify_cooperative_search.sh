#!/usr/bin/env bash
# Run cooperative-search verification inside a Terra Slurm allocation.
set -euo pipefail
test -n "${SLURM_JOB_ID:-}"
...
printf 'source=%s job=%s rustc=%s\n' "${ANNEAL_SOURCE_REV:?set ANNEAL_SOURCE_REV}" "$SLURM_JOB_ID" "$(rustc --version)"
case "${1:-full}" in
  regressions)
    status=0
