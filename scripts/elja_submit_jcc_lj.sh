...
readonly -a JCC_LJ_PROTOCOL_EXPORTS=(
  ...
  CATALOG_SEAM_PATIENCE=800
  CATALOG_SEAM_TRACE=0
  CATALOG_HISTO_RADIUS=1.4
  CATALOG_PROBE_INTERVAL=8
  CATALOG_PROBE_SCALE=0.2
  CATALOG_BRIDGE_INTERVAL=64
  QUENCH_NOISE=0
)
protocol_exports=$(IFS=,; printf '%s' "${JCC_LJ_PROTOCOL_EXPORTS[*]}")
...
for n in 38 55 75 98 102 104; do
  ...
  for arm in shared control; do
    ...
    sbatch \
      ...
      --mem="${ELJA_MEM:-16G}" \
      --output="$log" \
      --export="ALL,CATALOG_CAMPAIGN=${CAMPAIGN},SEED_OFFSET_BASE=${SEED_OFFSET_BASE},JCC_QUALIFIER_PYTHON=${QUALIFIER_PYTHON},${protocol_exports}" \
