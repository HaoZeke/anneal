#!/usr/bin/env bash
# Production of the packing-mean SOAP hop (Config::recommended).
# Paper budgets. Does not cancel any running HQ job.
set -euo pipefail
ROOT=${LJ_ROOT:-$HOME/anneal-build}
OUT=${LJ_OUT:-$HOME/ljwork/hq-sci-pack}
...
submit_arm() {
  ...
  hq submit \
    ...
    -- "$ONE" "$n" "$budget" rec | tee -a "$OUT/hq_submit.log"
