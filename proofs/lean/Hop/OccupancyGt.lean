...
the rematched family floor. CatalogSaturated does not retire. A
one-shot leftover nick is not a dwell. Mixing alone does not retire.
...
leftover dwell and the family floor. CatalogSaturated does not retire. -/
def retire (mixing packingSat leftoverDwell floorMet : Bool) : Bool :=
  mixing && packingSat && leftoverDwell && floorMet

/-- Mixing names a putative. It does not retire without packing
saturation. -/
theorem mixing_alone_does_not_retire (leftoverDwellMet floorMet : Bool) :
    retire true false leftoverDwellMet floorMet = false := by
  cases leftoverDwellMet <;> cases floorMet <;> rfl

/-- Paper floor: packing saturation with one rematched family does not
retire when `F = 2`. -/
theorem paper_floor_blocks_one_family :
    retire false true true false = false := by
  rfl

/-- Packing saturation with the family floor, even with leftover dwell,
does not retire. -/
theorem packing_sat_and_floor_does_not_retire :
    retire false true true true = false := by
  rfl
...
theorem leftover_unsaturated_does_not_retire :
    retire true true false true = false := by
  rfl

/-- Mixing plus packing saturation plus leftover dwell plus the floor
retires. -/
theorem mixing_and_packing_sat_retires :
    retire true true true true = true := by
  rfl
