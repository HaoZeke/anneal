...
A one-shot leftover nick is not a dwell. Mixing alone does not
retire. Many book families with extras already Left is not one cell.
...
leftover dwell (or one book family) and FunnelModel EI exhausted
and the family floor. CatalogSaturated does not retire. -/
