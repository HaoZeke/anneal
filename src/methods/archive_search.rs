...
pub fn archive_search<'g, R: Rng + ?Sized>(
    ...
    if cap < 50_000 {
        ...
        if molecular {
            // Full-budget walk: shorter quench and angular moves buy hops;
            // stall patience 8 re-places the packing when the chain sticks.
            // Splitting the ledger kept rec's one low isomer and never
            // found a second.
            let mut c = cfg.clone();
