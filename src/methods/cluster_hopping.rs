...
fn run_full<'g, R: Rng + ?Sized>(
    ...
    // The design vector the first stage was answered from, held until the
    // quench supplies the label. Carried rather than rebuilt so the model is
    // fitted on the vector it was consulted about.
    ...
    loop {
        ...
        if let Some(sur) = surrogate.as_mut() {
            ...
            match sur.predict_features(design.view(), tol) {
        ...
        if let (Some(sur), Some(design)) = (surrogate.as_mut(), pending_design.take()) {
            sur.observe_features(design.view(), e_new);
            // And the relaxed end of the same quench, which is its own answer.
            //
            ...
            // is a quenched minimum. Trained on one regime and consulted in
            // another it extrapolated, so every acceptance ratio was formed
            ...
            // for, and it is what anchors the model on the band the chain
            // actually lives in.
            //
            ...
            sur.observe_features(relaxed_design.view(), e_new);
        }
