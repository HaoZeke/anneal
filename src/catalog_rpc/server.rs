...
use crate::catalog::{
    ...
    explore_must_leave, invert_mixing, leftover_esty_stable, leftover_hatch_stable, leftover_lambda,
...
fn mixing_from_state(scientific: &ScientificState) -> MixingEvidence {
    ...
    if evidence.certified_attractor && families.len() >= 2 {
        let leftover = GoodTuringSample::from_counts(scientific.leftover_arrivals.values().copied());
        let leftover_ok = leftover_hatch_stable(
            leftover.n,
            leftover.n1,
            crate::catalog::PRODUCTION_MAX_UNSEEN_MASS,
        ) || scientific.leftover_dwell;
        if !leftover_ok {
            evidence.certified_attractor = false;
        }
    }
